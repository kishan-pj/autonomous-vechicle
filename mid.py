...
def calculate_alignment(cone1_xmax, cone2_xmin, screen_width):
    if cone1_xmax is None and cone2_xmin is None:
        # No cones detected, align with the center of the screen
        alignment_point = screen_width // 2
    elif cone1_xmax is None:
        # Only cone2 detected, align with its midpoint
        alignment_point = cone2_xmin
    elif cone2_xmin is None:
        # Only cone1 detected, align with its midpoint
        alignment_point = cone1_xmax
    else:
        # Both cones detected, align with the midpoint between them
        alignment_point = (cone1_xmax + cone2_xmin) // 2
    ...
    with tf.Session(graph=detection_graph) as sess:
        ...
        while True:
            ...
            if 'green' in detected_cone_colors and 'orange' in detected_cone_colors:
                ...
                alignment_point = calculate_alignment(cone1_xmax, cone2_xmin, frame.shape[1])

                if prev_cone1_xmax is not None and prev_cone2_xmin is not None:
                    prev_alignment_point = calculate_alignment(prev_cone1_xmax, prev_cone2_xmin, frame.shape[1])

                    prev_line_start = (prev_alignment_point, frame.shape[0])
                    prev_line_end = (prev_alignment_point, 0)
                ...
                line_start = (alignment_point, ground_y)
                line_end = (alignment_point, 0)
