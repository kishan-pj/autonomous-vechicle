...
def should_draw_line(cones):
    has_orange_cone = False
    has_green_cone = False

    for cone in cones:
        if cone['color'] == 'orange':
            has_orange_cone = True
        elif cone['color'] == 'green':
            has_green_cone = True

    return has_orange_cone and has_green_cone
...
def main():
    ...
    with tf.Session(graph=detection_graph) as sess:
        ...
        while True:
            ...
            detected_cones = []  # List to store the detected cones
            ...
            if should_draw_line(detected_cones):
                sorted_cones = sorted(detected_cones, key=lambda cone: cone['box'][1])
                cone1_xmax = sorted_cones[0]['box'][3]
                cone2_xmin = sorted_cones[1]['box'][1]
